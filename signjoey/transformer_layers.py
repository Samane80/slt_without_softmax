...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1):
        ...
        # softmax حذف شده؛ به‌جاش linear attention با feature map
        # squared-ReLU که در نهایت (فاز infer بعد از fold) فقط mult+add داره
        self.mode = "train"
    ...
    def _feature_map(x: Tensor) -> Tensor:
        """phi(x) = ReLU(x)^2 -- یک ضرب ساده‌ی یک عدد در خودش، بدون pow/exp واقعی."""
        r = torch.relu(x)
        return r * r

    def forward(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None):
        """
        Computes multi-headed linear attention (softmax با squared-ReLU
        feature map جایگزین شده).
        ...
        # همون scale قبلی حفظ شده؛ چون feature map غیرخطیه این فقط یک ثابت
        # گلوبال روی phi(q) میشه که در calibration داخل norm_const جذب میشه.
        q = q / math.sqrt(self.head_size)

        q_phi = self._feature_map(q)  # B,H,Mq,Dh
        k_phi = self._feature_map(k)  # B,H,Mk,Dh
        ...
        if mask is not None:
            if is_causal:
                ...
                key_mask = mask.squeeze(1)
            key_mask = key_mask.unsqueeze(1).unsqueeze(-1)  # [B,1,Mk,1]
            k_phi = k_phi.masked_fill(~key_mask, 0.0)
            v = v.masked_fill(~key_mask, 0.0)

        if not is_causal:
            # non-causal: یک بار sum روی کل بعد کلید کافیه
            kv = torch.einsum("bhtd,bhte->bhde", k_phi, v)
            ...
            kv_terms = torch.einsum("bhtd,bhte->bhtde", k_phi, v)
