...
class SoftmaxMultiHeadedAttention(nn.Module):
    """
    The original Multi-Head (Softmax) Attention from "Attention is All
    You Need" (implementation modified from OpenNMT-py,
    https://github.com/OpenNMT/OpenNMT-py), kept unchanged.

    Used only by the Decoder (`TransformerDecoderLayer`): target
    sequences are short (bounded by `translation_max_output_length`),
    so the O(N^2) cost of Softmax attention is negligible there, while
    Softmax's well-understood, stable training behavior is preserved
    exactly where CTC recognition doesn't depend on it. The Encoder
    (`TransformerEncoderLayer`), which processes much longer sign-video
    frame sequences and feeds the CTC recognition branch, keeps using
    `MultiHeadedAttention` (Focused Linear Attention + DWC) instead,
    where the O(N) complexity actually pays off.
    """
    ...
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1):
        ...
        :param dropout: probability of dropping a unit
        """
        super(SoftmaxMultiHeadedAttention, self).__init__()
